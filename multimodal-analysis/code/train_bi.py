...
class DatasetBI(Dataset):
    def __init__(self, train_or_test='train'):
        print('hh')
        self.trOte = train_or_test
        self.bi_dataset = []  # num: 0; 1: url; 2:text; 3:label
        self.bi_test_dataset = []
        with open("../../dataset/train_bi.csv", newline='', encoding='UTF-8') as f:
            reader = csv.reader(f)
            for row in reader:
                self.bi_dataset.append(row)

        with open("../../dataset/test_bi.csv", newline='', encoding='UTF-8') as f:
            reader = csv.reader(f)
            for row in reader:
                self.bi_test_dataset.append(row)

        self.train_set = [[i, s[0], int(s[1])] for i, s in enumerate(self.bi_dataset) if s[0] != '']
        self.test_set = [[i, s[0], int(s[1])] for i, s in enumerate(self.bi_test_dataset) if s[0] != '']

        self.train_avail = []
        self.test_avail = []

        for num, url, label in self.train_set:
            if os.path.exists(f"../../爬虫/bi_set/{label}/{num}.html"):
                self.train_avail.append([num, url, label])
        for num, url, label in self.test_set:
            if os.path.exists(f"../../爬虫/bi_test_set/{label}/{num}.html"):
                self.test_avail.append([num, url, label])
    ...
    def __getitem__(self, idx):
        if self.trOte == 'train':
            num = self.train_avail[idx][0]
            url = self.train_avail[idx][1]
            label = self.train_avail[idx][2]
            path = f"../../爬虫/bi_set/{label}/{num}.html"

            if not os.path.exists(f'../bi_cls_vectors/{label}/{num}.pth'):
                ...
                torch.save(features.clone(), f'../bi_cls_vectors/{label}/{num}.pth')
            else:
                features = torch.load(f'../bi_cls_vectors/{label}/{num}.pth')

            return features, url, label

        else:
            num = self.test_avail[idx][0]
            url = self.test_avail[idx][1]
            label = self.test_avail[idx][2]
            path = f"../../爬虫/bi_test_set/{label}/{num}.html"
            # print('not prepared yet')
            if not os.path.exists(f'../bi_cls_vectors/{label}/te-{num}.pth'):
                with open(path, encoding='utf-8') as f:
                    html = f.read()
                wp = Webpage(url)
                wp.html = html
                wp.is_valid = True
                features = self.H2V.train_get_score(wp)
                torch.save(features.clone(), f'../bi_cls_vectors/{label}/te-{num}.pth')
            else:
                features = torch.load(f'../bi_cls_vectors/{label}/te-{num}.pth')
            return features, url, label

    def __len__(self):
        if self.trOte == 'train':
            return len(self.train_avail)
        else:
            return len(self.test_avail)


class Url_bi_Dataset(Dataset):
    def __init__(self, w2idx, train_or_val='train'):
        super(Url_bi_Dataset, self).__init__()
        self.flag = train_or_val
        self.train = []
        self.valid = []

        with open("../../aligned_dataset/bi/train.csv", newline='', encoding='UTF-8') as f:
            reader = csv.reader(f)
            for row in reader:
                self.train.append(row)
        with open("../../aligned_dataset/bi/val.csv", newline='', encoding='UTF-8') as f:
            reader = csv.reader(f)
            for row in reader:
                self.valid.append(row)
        self.train = self.train[:10000]
        self.valid = self.valid[:10000]
...
class EarlyStopping:
    ...
    def save_checkpoint(self, f1, model, path):
        if self.verbose:
            print(
                f'F1 increased ({self.f1_max:.6f} --> {f1:.6f}).  Saving model ...model_new{MODEL_TYPE}_dev_bi.pth')
        if self.st:
            model.save(path)
        else:
            # 存在了这里
            torch.save(model.state_dict(), os.path.join(path, f'model_transformer_bi.pth'))
        self.f1_max = f1
...
class VanillaInstructor:
    def __init__(self):
        ...
        self.model_dev = WebsiteClassifier(use_my_model=True, try_type=MODEL_TYPE).model_dev_bi.to(device)

        if os.path.exists(f'./model_result/model_new{MODEL_TYPE}_dev_bi.pth'):
            print("已加载训练模型，开始断点续训")
            state_dict = torch.load(f'./model_result/model_new{MODEL_TYPE}_dev_bi.pth')
    ...
    def train_run(self):
        ...
        train_dataset = DatasetBI(train_or_test='train')
        test_dataset = DatasetBI(train_or_test='test')
        ...
        test1_size = int(len(test_dataset) * 0.2)
        test2_size = len(test_dataset) - test1_size
        # print(train_size)
        # print(test_size)

        # test_dataset, _ = torch.utils.data.random_split(test_dataset, [test1_size, test2_size])
        ...
        for epoch in range(self.epochs):
            ...
            with open(f"./log_res_csv/bi_log_res_{MODEL_NAME[MODEL_TYPE]}.csv", "a", encoding="UTF-8") as f:
                f.write(
    ...
    def pure_url_experiment(self, voc2idx, voc_size):
        ...
        train_dataset = Url_bi_Dataset(voc2idx)
        test_dataset = Url_bi_Dataset(voc2idx, 'valid')

        # train_sampler = make_weights_for_balanced_classes_split(train_dataset)

        train_dataloader = DataLoader(dataset=train_dataset, batch_size=16, shuffle=True, collate_fn=collate_fn)
        valid_dataloader = DataLoader(dataset=test_dataset, batch_size=16, shuffle=True, collate_fn=collate_fn)
        model_t = TransformerClassifier(output_dim=2, dropout=0.3, vocab_size=voc_size, num_layers=3).to(device)
        ...
        for epoch in range(self.epochs):
            ...
            acc = metrics.accuracy_score(t_targets_all.cpu().tolist(), t_outputs_all.cpu().tolist())
            print(f'acc: {acc}')
...
def make_dict():
    ...
    for _url, _label in tqdm(all_data):
        for c in _url:
            dictionary[c] += 1
