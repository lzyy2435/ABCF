...
class Get_CNSN(nn.Module):
    def __init__(self,cnsn_type,in_channels,crop,beta):
        super(Get_CNSN, self).__init__()
        if 'cn' in cnsn_type:
        ...
        if 'sn' in cnsn_type:
            ...
            self.selfnorm = SelfNorm(in_channels)
        else:
            self.selfnorm = None
        self.cnsn = CNSN(crossnorm=self.crossnorm, selfnorm=self.selfnorm)
