...
class TextualExtractor:
    ...
    def __init__(self, device='cpu', use_my=False):
        if not TextualExtractor.xlmr:
            if use_my is False:
                TextualExtractor.xlmr = SentenceTransformer('../pretrained_model', device=device)
            else:
                TextualExtractor.xlmr = SentenceTransformer('./xlm_text/', device=device)
        # self.xlmr = SentenceTransformer('xlm-r-distilroberta-base-paraphrase-v1', device=device)

        # TLD used for one-hot encoding
        # self.rep_tld_org = ['com', 'org', 'net', 'info', 'xyz', 'club', 'biz', 'top', 'edu', 'online',
        #                     'pro', 'site', 'vip', 'icu', 'buzz', 'app', 'asia', 'su', 'gov', 'space']

        self.rep_tld = ['com', 'org', 'net', 'info', 'xyz', 'cn', 'm4a', 'top', 'local', 'jpg',
                        'shop', 'cc', 'vip', 'co', 'live', 'lan', 'test', 'link', 'store', 'me']
    ...
    def get_features(self, url, html, text=None):
        features = {}
        ...
        if html is not None:
            ...
            if text is not None:
                features['f_text_' + str(self.k_sentences)] = my_embed_text(text, TextualExtractor.xlmr)
            else:
                # text
            ...
            features['f_metatags'] = None
            features['f_title'] = None
            features['f_description'] = None
            features['f_keywords'] = None
            features['f_links_' + str(self.k_links)] = None

            if text is not None:
                features['f_text_' + str(self.k_sentences)] = my_embed_text(text, TextualExtractor.xlmr)
            else:
                features['f_text_' + str(self.k_sentences)] = None

        return features


def my_embed_text(texts, transformer):
    sentences_trunc = [trunc(s, transformer.tokenizer, transformer.max_seq_length) for s in texts]
...
if __name__ == '__main__':
    te = TextualExtractor(use_my=True)
